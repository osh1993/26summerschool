...
2. Apps Script `프로젝트 설정 > 스크립트 속성`에 추가:
   ...
   - `CAMP_INTERNAL_TOKEN_VERSION` (선택) = 쓰기 토큰 세대. 미설정이면 `1`. 토큰 유출·즉시 전원 로그아웃이 필요하면 값을 바꾸면(예 `1`→`2`) 이전 발급 토큰이 전부 즉시 무효화(revoked)되고 관리자는 재로그인해야 한다. 쓰기 토큰은 발급 후 **30분** 만료된다(PII 노출창 축소).
...
6. (선택) 같은 `설정` 탭의 **배정 편집(조 · 방 · 차량)** 서브섹션에서 시트를 열지 않고도 참가자를 조/방/차량(운행)에 **배정·이동·해제**할 수 있다. 드래그 없이 `참가자` 선택 → `대상` 선택 → `배정 / 이동` 버튼으로만 조작한다(대상을 비우면 해제). **저장 시 정원·성별·중복·좌석(운전자 포함) 검증으로 위반은 차단**되고, 권장인원 초과·성별 과편중·잠금 이동·시간창 위반은 경고로 커밋된다. `locked`(잠금) 배정은 자동 제안이 덮어쓰지 못한다. 모든 변경은 `Change_Log`에 내부 ID만 남는다(PII 없음). **공개 반영은 자동이 아니며, `6. 공개 스냅샷 게시`를 다시 실행**해야 한다. 상세: `apps-script/README.md` 4-4장.
...
- 배정 편집(토큰 비밀키 설정 시): 로그인 후 조/방/차량 배정·이동·해제 → `Group_Assignments`/`Room_Assignments`/`Trip_Passengers`/`Change_Log`에 반영. 정원 초과·성별 불일치·중복·좌석 초과(운전자 포함)는 저장이 차단됨. 해제는 배정 행만 제거하고 조·방·차량·참가자 정의 행은 보존. 편집 직후 공개 페이지는 그대로이며, `공개 스냅샷 게시`를 다시 실행해야 공개에 반영됨. `get_assignments` 응답 소스에 전화·생년월일 등 민감필드가 없음.
