...
- 성공 시에만 `internal-snapshot/v1`(공개 v2 구조 + 각 member `full_name` + 최상위 `teachers[]`/`staff[]`)을 반환한다. 실패·미설정·오류 시에는 `{ "error": "unauthorized" }` 또는 `{ "error": "temporarily_unavailable" }`만 반환하며 힌트/스택이 없다.
