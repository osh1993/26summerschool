...
## 4-2. 웹 관리자 설정 탭(설정·매핑·조·방·차량 편집)

내부 뷰에 로그인한 교역자·운영자는 공개 페이지의 **`설정` 탭**에서 시트를 직접 열지 않고도 운영 설정과 조/방/차량, Form 필드 매핑을 웹에서 편집할 수 있다. 저장은 인증된 **쓰기 토큰**이 있을 때만 활성화된다.

### Script Property 설정: `CAMP_INTERNAL_TOKEN_SECRET`

1. Apps Script 편집기 `프로젝트 설정(톱니바퀴) > 스크립트 속성`을 연다.
2. 속성 `CAMP_INTERNAL_TOKEN_SECRET`에 **임의의 긴 무작위 문자열**(예: 40자 이상)을 넣는다. 이 값은 쓰기 토큰의 HMAC-SHA256 서명 비밀키다.
   - PowerShell로 임의 비밀키 생성 예:
     ```powershell
     -join ((48..57) + (65..90) + (97..122) | Get-Random -Count 48 | ForEach-Object { [char]$_ })
     ```
3. **미설정이면 저장이 안전 기본값으로 잠긴다.** 로그인은 되어 내부 명단은 볼 수 있지만, `설정` 탭은 "토큰 비밀키 미설정" 안내와 함께 저장이 비활성화된다(읽기·공개 뷰에는 영향 없음).
4. 값을 저장소·코드에 커밋하지 않는다(여기에는 이름만 문서화). 새 코드 반영을 위해 **새 버전으로 재배포**한다.

### 동작·인증

- 로그인 성공 시 서버가 `issueAuthToken`으로 **2시간 만료 토큰**을 함께 반환한다. 브라우저는 이 토큰만 `sessionStorage`(`camp.internal.token`)에 임시 보관하고, 비밀번호는 저장하지 않는다.
- 저장 요청은 `/exec`로 `{action, token, payload}`를 POST한다. 서버는 매 요청 `verifyAuthToken`으로 토큰을 검증하고, 통과한 뒤에만 `LockService` 문서 잠금 안에서 시트에 쓴다.
- 토큰이 만료/변조되면 `token_expired`/`unauthorized`로 거부되며 화면은 재로그인을 안내한다. 비밀키 미설정이면 `writes_disabled`로 거부한다.
- 로그인 실패가 짧은 시간에 반복되면(글로벌 카운터, 10분 내 10회 초과) 잠시 로그인을 거부한다(브루트포스 경량 완화).

### 편집 범위(안전 경계)

- **운영 설정:** `EVENT_NAME`, `EVENT_START_DATE`, `EVENT_END_DATE`, `GROUP_COUNT`, `ROOM_COUNT`, `ATTENDANCE_SOURCE_HEADER`, `RAW_STUDENT_SHEET`, `RAW_STAFF_SHEET`만 편집 가능(화이트리스트). 동기화 커서·게시 상태·행사 ID 같은 상태 키는 웹에서 편집할 수 없다. "조 개수 맞추기"/"방 개수 맞추기" 버튼은 메뉴의 `3-1`/`4-1`과 동일 로직을 호출한다.
- **필드 매핑:** 외부 참조가 없어 전체 교체 방식이다. `source_sheet`는 `Form_Raw_Students`/`Form_Raw_Staff`/`Roster_Import`, `normalized_field`는 정규 어휘만 허용된다.
- **조·방·차량:** `id`를 보존하는 in-place 편집이다. 기존 행은 값만 수정하고, `id` 없는 행만 새 `id`를 발급해 추가한다. 목록에서 뺀 기존 행은 **물리 삭제하지 않고**, 배정(Group_Assignments/Room_Assignments/Trips/Vehicle_Availability) 참조가 있으면 보존+경고, 없으면 `active=FALSE`로 비활성화한다. 배정이 있는 행은 화면에서 "배정 있음"으로 표시되고 비활성 토글이 잠긴다.
- 저장 응답은 `{ok:true, warnings:[...]}` 또는 힌트 없는 오류 코드만 돌려주며, 스택·시트명·셀 범위·개인정보를 포함하지 않는다.
- `docs/config.js`의 `internalApiUrl`이 비어 있으면(샘플 모드) `설정` 탭은 폼을 비활성 상태로만 보여 주고 저장은 막힌다.

## 4-3. 웹 참석자 관리(참석자 CRUD — PII 포함)

`설정` 탭의 **참석자 관리** 서브섹션에서, 인증한 교역자·운영자가 시트를 열지 않고도 참석자를 웹에서 **추가·수정·비활성**할 수 있다. 4-2와 동일한 쓰기 토큰·잠금·비밀키 게이트를 그대로 재사용한다.

### PII 표시 주의 (인증 브라우저 전용)

- 이 화면은 조·방·차량 편집과 달리 **실명(`legal_name`)과 연락처(`phone`/`birth_date`/`guardian_phone`/`insurance_status`/`private_note`)를 인증 브라우저에 직접 표시**한다. 화면 상단에 경고 배너로 이를 고지한다.
- 참석자 데이터(`get_participants`)는 토큰 인증자에게만 서버에서 반환되며 **정적 파일로 저장하지 않는다.** 공개 스냅샷(`doGet view=public`)과 GitHub Wiki에는 절대 노출되지 않는다.
- 민감필드는 `Participant_Private`로만 라우팅된다. 공개 후보 테이블(`Participants`)에 민감필드가 섞여 오면 Core 검증이 `PARTICIPANT_PRIVATE_LEAK`로 차단한다.
- 공용 PC·공유 화면에서 이 탭을 열지 않는다. 로그아웃하면 목록·편집 폼과 토큰이 즉시 지워진다.

### 동작(액션)

- `get_participants`: 편집용 목록(공개모델 + `legal_name` + `active`)과 민감필드 맵(`private`)을 반환. 토큰 인증 전용.
- `save_participant`: `{participant, private}` 저장. `participant_id`가 기존 행에 있으면 **in-place 수정**(id 불변: `participant_id`/`public_id`/`source_response_id`는 절대 변경하지 않음), 없으면 **신규 발급**(`pt_`+UUID / `generatePublicId_`). 신규는 `public_consent=FALSE`·`public_name=''`·`active=TRUE`가 기본이며, 공개 노출은 기존 동의·성 마스킹 파이프라인 규칙을 그대로 따른다(자동 공개 강제 없음).
- `deactivate_participant`: `{participant_id}` → **삭제가 아니라 `active=FALSE`만** 설정한다(배정·출석 참조 보존). 확인 후에만 실행된다.

### Change_Log 감사

- 모든 변경은 `Change_Log`에 남는다. `changed_by`는 **토큰의 사용자**, `reason`은 `admin_web`이다. `change_id`/`changed_at`는 서버에서 발급한다.
- 공개필드 변경은 old/new 원값으로 기록하되, **민감필드는 old/new 모두 `[REDACTED]`**로만 남겨 연락처 원문이 로그에 남지 않게 한다.
- 비활성 처리는 `entity_type='participant'`, `field_name='active'`, `TRUE→FALSE`로 감사된다.

### 안전 경계

- 검증 실패는 `invalid_input`(필드별 안내 메시지), 세션 만료/변조는 `token_expired`/`unauthorized`, 비밀키 미설정은 `writes_disabled`로 거부하며 스택·시트·셀·PII를 노출하지 않는다.
- `internalApiUrl` 미설정(샘플 모드)에서는 참석자 관리가 **비활성**되고 "실서버에서만 조회·편집" 안내만 보인다. 실데이터·PII는 실서버에서만 표시된다.

## 4-4. 웹 배정 편집(조·방·차량 배정)

`설정` 탭의 **배정 편집(조 · 방 · 차량)** 서브섹션에서, 인증한 교역자·운영자가 시트를 열지 않고도 참가자를 **조/방/차량(운행)에 배정·이동·해제**할 수 있다. 4-2/4-3과 동일한 쓰기 토큰·문서 잠금·비밀키 게이트를 그대로 재사용한다. 조·방·차량의 **정의**(개수·정원·성별범위)는 4-2에서 편집하고, 여기서는 그 위의 **사람↔슬롯 배정**만 다룬다.

### 조작 방식 (접근성 우선)

- **드래그를 쓰지 않는다.** `참가자` 선택 → `대상 조/방/운행` 선택 → `배정 / 이동` 버튼으로만 배정한다(키보드·스크린리더 친화). 대상 슬롯을 비우면(`(미배정으로 해제)`) **해제**된다.
- 슬롯별 카드에 현재 인원과 **정원/성별/좌석 잔여(운전자 포함)**를 함께 표시하고, 미배정 참가자 풀을 별도 카드로 보여준다. 각 인원의 `편집`은 폼을 채워 role·잠금(locked)을 바꿔 다시 저장할 수 있고, `해제`는 그 배정만 제거한다.
- 조·차량 배정은 `role`(조원/조장/부조장/조선생님)과 `잠금`을, 방 배정은 `잠금`을 지정할 수 있다.

### 동작(액션)

- `get_assignments`: 배정 편집에 필요한 데이터만 반환한다 — 참가자(공개모델 + `legal_name`, **`phone`·`birth_date` 등 민감필드 없음**, `Participant_Private`를 읽지 않음), 조·방·차량 정의, 현재 조/방/차량 배정. 방의 `private_note`는 제외된다.
- `save_group_assignment` `{participant_id, group_id|null, role?, locked?}` / `save_room_assignment` `{participant_id, room_id|null, locked?}` / `save_trip_passenger` `{participant_id, trip_id|null, direction, seat_count?, boarding_status?, locked?}`: 각 저장은 문서 잠금 안에서 **Core 검증 → 차단 이슈가 없을 때만 커밋**한다.

### 저장 시 검증(차단 vs 경고)

- **차단(커밋 안 함, `{error:'validation_blocked', issues:[...]}`):** 존재/활성 조 참조, 조·방·차량 **중복 배정**(참가자당 조 1·방 1·direction당 운행 1), **방 정원 초과·성별 불일치**, **차량 정원 초과(운전자 좌석 포함)**, 운전자를 같은 운행의 승객으로 배정, 잘못된 role/direction, 비활성·취소 대상.
- **경고(커밋하되 응답 `warnings`에 반환):** 조 권장/최대 인원 초과, 조 성별 과편중, 잠금(locked) 배정을 이동하려 함, 차량 출발이 요청 시간창을 벗어남.
- 웹 UI는 차단 시 이슈 목록을 `aria-live`로 알리고 **서버 재조회로 화면을 롤백**한다. 성공 시 경고 건수를 함께 표시한다.

### 잠금(locked)의 의미

- `locked=TRUE` 배정은 자동 조편성·차량배치 제안(`replaceAutomatic*`)이 **덮어쓰지 못한다**(수동 저장만 변경 가능). 잠금된 배정을 다른 슬롯으로 옮기려 하면 경고를 반환하므로, 필요하면 먼저 잠금을 해제하고 저장한다.

### Change_Log 감사 · 삭제 금지

- 모든 배정 추가/이동/해제는 `Change_Log`에 남는다. `entity_type`은 `group_assignment`/`room_assignment`/`trip_passenger`, `changed_by`는 토큰 사용자, `reason`은 `admin_web`이며, **내부 ID만**(participant_id·group_id·room_id·trip_id) 기록하고 실명·연락처는 남기지 않는다.
- 해제는 **배정 행만** 물리 삭제한다. 참가자·조·방·차량의 **정의 행은 절대 삭제하지 않는다**.

### 공개 반영은 별도

- 배정 편집은 내부 데이터만 바꾼다. **공개 노출은 기존 `수련회 운영 > 6. 공개 스냅샷 게시`를 다시 실행**해야 반영된다(성 마스킹·동의·게시 검증 파이프라인 통과). Phase C는 자동으로 공개하지 않는다.
- `internalApiUrl` 미설정(샘플 모드)에서는 배정 편집이 **비활성**되고 안내만 보인다. 공개 읽기 탭(조편성/방배정/차량 운행)은 그대로 동작한다.
